...
WITH director_counts AS (
    SELECT YEAR(ns.date_added) AS year_added, nd.director, COUNT(*) AS cnt
    FROM netflix_stg ns
    JOIN netflix_directors nd ON ns.show_id = nd.show_id
    WHERE ns.type = 'Movie' AND ns.date_added IS NOT NULL
    GROUP BY YEAR(ns.date_added), nd.director
),
ranked AS (
    SELECT *, RANK() OVER (PARTITION BY year_added ORDER BY cnt DESC) AS rn
